...
anysearchTwitter = function(searchterm = "Anywhere",
                     n = 100000,
                     since = '2017-10-25',
                     until = '2017-10-26',
                     olddataname = "Puerto Rico 23 Sept - 25 OCT best.csv",
  ...
  if(file.exists(olddataname)==TRUE){
    ...
    df.tweet = df.tweet[which(!is.na(df.tweet$text)),]
    write.csv(df.tweet, newdataname)
    return(paste("Your New Data about", searchterm,"Has Been Added to the basefile and saved as", newdataname))
